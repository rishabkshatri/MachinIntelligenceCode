...
class KNN:
    ...
    def k_neighbours(self, x):
        ...
        for j in range(len(lni)):
	        indices = [j for j in range(self.data.shape[0])]
	        d = list(list(zip(*list(sorted(zip(lni[j], indices)))))[0])
        	e = list(list(zip(*list(sorted(zip(lni[j], indices)))))[1])
